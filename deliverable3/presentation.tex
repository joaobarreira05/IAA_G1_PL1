...
\begin{frame}{Autoencoders Training Convergence}
  \begin{columns}
    \begin{column}{0.6\textwidth}
      ...
      \scriptsize{Training and validation Loss curves for both network branches}
    \end{column}
    \begin{column}{0.4\textwidth}
      \textbf{Convergence Analysis:}
      \begin{itemize}
        \item Dynamic training using early stopping on validation reconstruction loss.
        \item Primary AE converged and stopped at epoch 25.
        \item Secondary AE converged and stopped at epoch 40.
...
\begin{frame}{Final Metric Comparison (Test Set)}
  \begin{table}
    ...
    \begin{tabular}{l|ccc|ccc}
      ...
      Second AE & \textbf{0.7615} & 0.7812 & \textbf{0.7712} & \textbf{0.9265} & 0.7812 & 0.8477 \\
      \textbf{Ensemble AE} & 0.5722 & \textbf{0.9710} & 0.7200 & 0.8448 & \textbf{0.9710} & \textbf{0.9035} \\ \hline
    \end{tabular}
  \end{table}
  \begin{itemize}
    \item \textbf{Deep Learning Dominance:} PyTorch Autoencoders far exceed classical.
    \item \textbf{Ensemble AE:} Best recall (\textbf{97.1\%}) and AUC-ROC (\textbf{0.947}), crucial for maximum security.
    \item \textbf{Second AE:} Best precision (\textbf{76.15\%} original) and F1 (\textbf{0.7712}), minimizing false alarms.
  \end{itemize}
\end{frame}
...
\begin{frame}{Ensemble Diagnostics}
  \begin{columns}
    ...
    \begin{column}{0.5\textwidth}
      ...
      \scriptsize{ROC Curve (AUC = 0.947)}
    \end{column}
  \end{columns}
  \vfill
  \begin{itemize}
    \item Out of 16,697 actual anomalies in the test set, only 484 are missed.
    \item The 57.22\% precision is due to the logical OR aggregating false positives, which is an acceptable compromise for security operations.
  \end{itemize}
\end{frame}
...
\begin{frame}{Conclusions and Future Work}
  \begin{itemize}
    ...
    \begin{enumerate}
      ...
      \item Ensemble AE provides maximal security monitoring (97.1\% recall).
